...
fun ReservationDetailScreen(
    ...
    // ✅ Correct way to fetch orders
    LaunchedEffect(reservation.id) {
        orderVM.fetchOrders(reservation.id)
    }
    ...
    Column {
        ...
        Column(
            ...
            Text(
                "Reservation ID: ${reservation.id.takeLast(12)}",
                fontWeight = FontWeight.ExtraBold,
                fontSize = 22.sp
            )
            Spacer(Modifier.height(4.dp))
            Text("Booking Date: ${dateFormat.format(reservation.timestamp)}")
            Text("Booking Time: ${timeFormat.format(reservation.timestamp)}")
            ...
            reservation.tables.forEach { table ->
                val seatCount = reservation.peopleCount[table] ?: 0
                    ...
                    orders.forEach { order ->
                        order.items.forEach { (itemId, item) ->
                            Row(
                                ...
                                IconButton(onClick = {
                                    orderVM.decreaseItemQuantity(order.id, reservation.id, itemId)
                                }) {
                                    Icon(Icons.Default.Delete, contentDescription = "Decrease Quantity", tint = Color.Red)
                                }
...
fun generatePDF(
    ...
    drawCenteredText("Reservation ID: ${reservation.id.takeLast(12)}", bodyPaint)
    drawCenteredText("Date: ${SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(reservation.timestamp)}", bodyPaint)
    drawCenteredText("Time: ${SimpleDateFormat("hh:mm a", Locale.getDefault()).format(reservation.timestamp)}", bodyPaint)
    ...
    reservation.tables.forEach { table ->
        reservation.seats[table]?.let { seats ->
...
fun printPDF(context: Context, file: File) {
    ...
    val printAdapter = PdfPrintDocumentAdapter(context, file)
    printManager.print("Order Slip", printAdapter, null)
}
